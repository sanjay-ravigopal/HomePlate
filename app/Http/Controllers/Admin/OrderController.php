...
class OrderController extends Controller
{
    ...
    public function evening_delivery_list($status, Request $request)
    {
        $key = explode(' ', $request['search']);
        ...
        $orders = Order::with(['customer', 'restaurant'])
            ->where('order_type', 'delivery_evening')
            ->when(isset($request->zone), function ($query) use ($request) {
                return $query->whereHas('restaurant', function ($q) use ($request) {
                    return $q->whereIn('zone_id', $request->zone);
                });
        ...
        return view('admin-views.order.evening_delivery_list', compact('orders', 'status', 'orderstatus', 'scheduled', 'vendor_ids', 'zone_ids', 'from_date', 'to_date', 'total', 'order_type'));
    }
